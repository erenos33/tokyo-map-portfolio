...
public class ReviewController {
    ...
    public ResponseEntity<Page<ReviewResponseDto>> getReviewsSorted(
    ...
    public ResponseEntity<Void> likeReview(
        ...
        return ResponseEntity.ok().build();// 바디 없음
    }

    @Operation(summary = "리뷰 좋아요 취소", description = "리뷰에 좋아요를 취소합니다. 로그인한 사용자만 가능합니다.")
    @SecurityRequirement(name = "bearerAuth")
    @DeleteMapping("/reviews/{id}/like")
    public ResponseEntity<Void> unlikeReview(@PathVariable Long id, @AuthenticationPrincipal String email) {
    ...
    public ResponseEntity<ReviewLikeCountResponseDto> getLikeCount(@PathVariable Long id) {
        return ResponseEntity.ok(reviewService.countLikesByReview(id));
    }
    ...
    public ResponseEntity<ReviewStatisticsResponseDto> getReviewStatistics(@PathVariable Long restaurantId) {
        return ResponseEntity.ok(reviewService.getReviewStatistics(restaurantId));
    }
